...
public:
  ...
  BDD compute_gb(const Cudd& manager, F1& system_post, F2& radius_post, F3&& avoid, size_t& no_trans) {
    ...
    size_t compute_controller_size(BDD& X){
        size_t nvars = m_pre.get_no_bdd_vars() +
                        m_input.get_no_bdd_vars();
        return X.CountMinterm(nvars);
    }
    
    
// MST
    template<class F1>
    BDD compute_gbLu(const Cudd& manager, F1& system_post, double tau, size_t& no_trans) {
        return compute_gbLu(manager, system_post, tau, [](const abs_type&) noexcept {return false;}, no_trans);
    }
    ...
    BDD compute_gbLu(const Cudd& manager, F1& system_post, double tau, F3&& avoid, size_t& no_trans) {
        /* number of cells */
        abs_type N=m_pre.size();
        /* number of inputs */
        abs_type M=m_input.size();
...
//                system_post(x,u);
   ...
   // MST
   BDD compute_gb(const Cudd& manager, size_t& no_trans, const double tau) {
        return compute_gb(manager, [](const abs_type&) noexcept {return false;}, no_trans, tau);
    }
    /**
     * @brief computes the transition function
     *
     ...
     **/
    template<class F3>
    BDD compute_gb(const Cudd& manager, F3&& avoid, size_t& no_trans, const double tau) {
        /* number of cells */
        abs_type N=m_pre.size();
        /* number of inputs */
        abs_type M=m_input.size();
